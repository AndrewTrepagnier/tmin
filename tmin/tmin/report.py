...
# Default template lives next to this package
_DEFAULT_TEMPLATE_DIR = Path(__file__).resolve().parent.parent / "Engineering Memorandum Template"
_DEFAULT_TEMPLATE_PATH = _DEFAULT_TEMPLATE_DIR / "ENGINEERING MEMORANDUM.txt"
...
def _get_input_json_path(root: Path, input_dirname: str = "Input") -> Path:
    """Return path to first .json in Input/. Raises FileNotFoundError if none."""
...
def run_report_from_input(
    ...
    *,
    ...
    """
    Run tmin and write memorandum and tmin report to OUTPUTS/.

    Use values= to pass an in-memory dict. Otherwise reads the first .json from Input/.
    If root is None, searches upward from cwd for a directory containing Input/.
...
def report(
    ...
    *,
    ...
    """
    Generate the engineering memorandum and write it to OUTPUTS/.

    Use data= or values= to pass an in-memory dict (e.g. from a notebook). Otherwise
    reads the first JSON from Input/.

    If root is None, the project root is found by searching upward from the
    current directory for a folder named Input/.
    ...
    if verbose:
        print("Report written to OUTPUTS/")
...
def run(root: Optional[Path] = None, verbose: bool = True) -> Dict[str, Any]:
    """Alias for tmin.report(). Writes the memorandum to OUTPUTS/."""
